...
class Comments {
    ...
    public static function loadComments($links, $name_image,$material,$size,$red_eyes,$cut,$text_comment) {
            $text_comment = htmlspecialchars($text_comment); //проверка на спецсимволы
            $name_image = htmlspecialchars($name_image); //проверка на спецсимволы
            $comments = fopen("../$links"."Comment.txt", "a+t"); //создание текстового документа
            fwrite($comments, "Name image: ".$name_image."\nMaterial: ".$material.
